...
class ExtratorZelt(ExtratorBase):
    ...
    def extrair(self, texto, link):
        ...
        # quartos / dormitórios (variações de texto)
        encontrados = re.findall(r"(\d+)\s*(?:quartos?|dormi[tóo]rios?|dorms?)", texto.lower())
        if encontrados:
            quartos = int(encontrados[0])



        # título — escolha a primeira linha significativa
        ...
        suites = 0
        vagas = 0
        # banheiros, suites, vagas (variações)
        ...
        area_priv = 0
        area_total = 0
        # áreas (privativa / total) - aceitar separadores e variações
